...
/** Hides one pack from learner-facing discovery after ADMIN authorization. */
export async function hidePackAction(input: unknown): Promise<ActionResult> {
  ...
  try {
    await packRepository.hide(parsed.data.id, session.user.id, getRequestIp(requestHeaders));
    revalidatePath("/admin/packs");
    revalidatePath(`/admin/packs/${parsed.data.id}/edit`);
    return { success: true, message: "Pack hidden." };
  } catch {
    return { success: false, message: "Unable to hide pack." };
  }
}
