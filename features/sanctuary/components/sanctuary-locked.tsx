...
export function SanctuaryLocked({
  reference,
  waypointId,
}: {
  reference: string;
  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-linear-to-b from-violet-50 via-background to-emerald-50 px-4 dark:from-violet-950 dark:via-slate-950 dark:to-emerald-950">
      <section className="w-full max-w-sm rounded-[2rem] border border-violet-300/30 bg-card/90 p-7 text-center shadow-xl">
        ...
        <h1 className="mt-2 font-heading text-3xl font-black">Practice in progress</h1>
        <p className="mt-3 text-sm text-muted-foreground">Sanctuary reopens after Radiance.</p>
        <Link
        ...
        >
          <MapIcon aria-hidden="true" /> Return to journey
        </Link>
      </section>
    </main>
  );
