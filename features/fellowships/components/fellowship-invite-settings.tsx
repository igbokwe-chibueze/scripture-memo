  ...
  return (
    <section className="mt-6 rounded-[2rem] border border-amber-400/25 bg-card p-5 shadow-xl sm:p-7">
      <div className="flex items-start gap-3">
        ...
        <div><h2 className="font-heading text-xl font-bold">{t("inviteSettingsTitle")}</h2><p className="mt-1 text-sm text-muted-foreground">{t("inviteSettingsDescription")}</p></div>
      </div>
      <div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/8 p-4">
        <p className="text-xs font-bold tracking-wider text-amber-700 uppercase dark:text-amber-300">{t("inviteCode")}</p>
        <code className="mt-2 block break-all font-mono text-base font-bold text-foreground">{inviteCode}</code>
      </div>
