...
export function FellowshipDirectory({ data, initialSearch, initialInviteCode }: { data: FellowshipDirectoryData; initialSearch: string; initialInviteCode: string }): React.ReactNode {
  ...
  const join = (fellowshipId: string): void => runMutation(fellowshipId, async () => {
    handleResult(await joinFellowshipAction({ fellowshipId }));
  });
  const requestJoin = (fellowshipId: string): void => runMutation(fellowshipId, async () => {
    handleResult(await requestFellowshipJoinAction({ fellowshipId }));
  });
  const cancelRequest = (requestId: string): void => runMutation(requestId, async () => {
    const result = await cancelFellowshipJoinRequestAction({ requestId });
    if (!result.success) {
  ...
  const joinInvite = (): void => runMutation("invite", async () => {
    handleResult(await joinFellowshipByInviteAction({ inviteCode }));
  });
