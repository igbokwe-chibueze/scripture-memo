...
import { KeyRoundIcon, SearchIcon, ShieldCheckIcon, UsersRoundIcon } from "lucide-react";
...
function FellowshipCard({ fellowship, onJoin, pending }: { fellowship: FellowshipSummary; onJoin: (id: string) => void; pending: boolean }): React.ReactNode {
  ...
  return (
    <article className="rounded-[1.75rem] border border-violet-300/25 bg-card/90 p-5 shadow-lg shadow-violet-950/8">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-1 rounded-full bg-muted px-3 py-1 text-xs font-black"><UsersRoundIcon className="size-3.5" /> {t("members", { count: fellowship.memberCount })}</span>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        {fellowship.isMember ? <NavigationButton className="col-span-2" href={`/fellowships/${fellowship.slug}`} pendingLabel={t("opening")}>{t("open")}</NavigationButton> : <><NavigationButton variant="outline" href={`/fellowships/${fellowship.slug}`} pendingLabel={t("opening")}>{t("viewFellowship")}</NavigationButton><Button disabled={pending} onClick={() => onJoin(fellowship.id)}>{t("join")}</Button></>}
      </div>
    </article>
  );
}
  ...
  return (
    <>
      ...
      <section className="mt-9" aria-labelledby="my-fellowships"><div className="flex items-center justify-between gap-3"><h2 id="my-fellowships" className="font-heading text-2xl font-black">{t("yourFellowships")}</h2><NavigationButton href="/fellowships/new" pendingLabel={t("opening")}>{t("create")}</NavigationButton></div>
        {data.memberships.length ? <div className="mt-4 grid gap-4 md:grid-cols-2">{data.memberships.map((item) => <FellowshipCard key={item.id} fellowship={item} onJoin={join} pending={isPending} />)}</div> : <div className="mt-4 rounded-3xl border border-dashed p-8 text-center"><ShieldCheckIcon className="mx-auto size-10 text-violet-500" /><h3 className="mt-3 font-heading text-xl font-black">{t("noneYet")}</h3><p className="mt-1 text-muted-foreground">{t("nonePrompt")}</p></div>}
      </section>
      ...
      <section className="mt-10" aria-labelledby="discover-fellowships"><h2 id="discover-fellowships" className="font-heading text-2xl font-black">{t("discover")}</h2><form className="relative mt-4"><SearchIcon className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" /><Input name="q" defaultValue={initialSearch} placeholder={t("search")} className="min-h-12 rounded-2xl pl-12" /></form>
        {data.publicFellowships.length ? <div className="mt-4 grid gap-4 md:grid-cols-2">{data.publicFellowships.map((item) => <FellowshipCard key={item.id} fellowship={item} onJoin={join} pending={isPending} />)}</div> : <p className="mt-5 rounded-2xl bg-muted p-6 text-center text-muted-foreground">{t("noPublic")}</p>}
      </section>
