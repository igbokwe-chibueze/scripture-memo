  ...
  return (
    <article className="rounded-[1.75rem] border border-violet-300/25 bg-card/90 p-5 shadow-lg shadow-violet-950/8">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-2"><span className="inline-flex items-center gap-1 rounded-full bg-muted px-3 py-1 text-xs font-bold"><UsersRoundIcon className="size-3.5" /> {t("members", { count: fellowship.memberCount })}</span><span className="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-bold">{fellowship.isPublic ? <UserRoundCheckIcon className="size-3.5" /> : <LockKeyholeIcon className="size-3.5" />}{fellowship.isPublic ? t("publicShort") : t("privateShort")}</span></div>
      </div>
      ...
      <div className="mt-5 grid gap-2">
        {fellowship.isMember ? (
          ...
          <>
            <span className="inline-flex min-h-11 items-center justify-center rounded-xl bg-violet-500/10 px-3 text-center text-sm font-bold text-violet-600 dark:text-violet-300">
              {t("requestPending")}
            </span>
            <LoadingButton
              variant="outline"
  ...
  return (
    <>
      <section className="rounded-[2rem] border border-amber-300/25 bg-linear-to-br from-violet-950 via-slate-950 to-amber-950 p-5 text-white shadow-xl sm:p-7">
        <div className="flex items-center gap-3"><KeyRoundIcon className="size-6 text-amber-300" /><div><h2 className="font-heading text-xl font-bold">{t("haveInvite")}</h2><p className="text-sm text-slate-300">{t("invitePrompt")}</p></div></div>
        <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto]">
          ...
          <LoadingButton
            ...
            onClick={joinInvite}
            className="min-h-12 bg-amber-400 font-bold text-slate-950 hover:bg-amber-300"
          >
            {t("joinWithCode")}
          </LoadingButton>
        </div>
      ...
      <section className="mt-9" aria-labelledby="my-fellowships"><div className="flex items-center justify-between gap-3"><h2 id="my-fellowships" className="font-heading text-2xl font-bold">{t("yourFellowships")}</h2><NavigationButton href={transport.linkHref("/fellowships/new")} pendingLabel={t("opening")}>{t("create")}</NavigationButton></div>
