...
| 14.7 | Security headers are configured in `next.config.ts` | 🟡 Medium | 🟡 In progress | X-Frame-Options, X-Content-Type-Options, Referrer-Policy, and Permissions-Policy are configured. Proxy attaches per-document nonce-based CSP in report-only mode, allows same-origin lazy chunks, and permits only the two exact inline style hashes observed from Sonner 2.0.7. Zod browser schemas use `jitless` to avoid eval probing. Owner confirmed `/game/map`, `/`, `/login`, `/register`, the prepared gameplay preview, and Oil Shop tab switching were clear in production on 2026-09-25. Next: check theme changes in Settings. |
