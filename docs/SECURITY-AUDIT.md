...
| 2.8 | Email verification is enforced for email/password registrations | 🟡 Medium | ☑ Implemented | Better Auth requires verification before session creation; valid-credential sign-in resends the link; only the latest unexpired link is accepted and it is consumed after use, enforced by a keyed digest in the existing Verification table; local Light Dev downloads the one-hour link, production uses Resend; pre-rollout accounts are grandfathered by the local-only rollout migration. Production Resend credentials and sender-domain verification remain deployment configuration, and owner browser acceptance is pending |
