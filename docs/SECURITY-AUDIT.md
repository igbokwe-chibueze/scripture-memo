...
| 2.5 | Login errors use generic messages — do not reveal whether an email exists | 🟡 Medium | ☑ Implemented | Invalid credentials remain generic; Better Auth returns a generic registration result for duplicate addresses while email verification is required |
...
| 2.8 | Email verification is enforced for email/password registrations | 🟡 Medium | ☑ Implemented | Better Auth requires verification before session creation; valid-credential sign-in resends the link; only the latest unexpired link is accepted and consumed after use, enforced by a keyed digest in the existing Verification table. Owner confirmed latest-link and replay browser checks on 2026-09-26; direct protected-route denial while unverified remains to verify. Local Light Dev downloads one-hour links, production uses Resend, and pre-rollout accounts are grandfathered by the local-only migration. Production Resend credentials and sender-domain verification remain deployment configuration |
