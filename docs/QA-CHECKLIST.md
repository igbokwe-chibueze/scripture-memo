...
| 7 | Cooldown bypass | On an administrator's active cooldown card, **Verify server lock** calls the real session-start action while the client card remains locked. Only the server's specific `PRG-004` cooldown response produces the successful verification notice; no day or session starts. | Not run |
| 8 | Radiance completion | The third flame, Glow reward, waypoint completion, and next published waypoint unlock occur together. | Not run |
| 9 | Duplicate reward prevention | Repeating a completion request does not duplicate Glow, Beacon XP, badges, or ledger entries. | Not run |
| 10 | Badge trigger | A trusted qualifying event updates or unlocks the expected badge once and shows ordered celebration/notice feedback. | Not run |
